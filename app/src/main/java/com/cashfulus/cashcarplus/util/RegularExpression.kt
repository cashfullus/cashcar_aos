...
// 전화번호 길이 : 12자~13자
val phoneLengthRE = "^(?=.{12,13}\$).*"

// 년도 : 2018
val yearRE = "^[1-2][0-9][0-9][0-9]\$"
// 생일 : 860413
val birthRE = "^[0-9][0-9][0-1][0-9][0-3][0-9]\$"
...
fun String.isValidPhone(): Boolean = this.isNotEmpty() && Patterns.PHONE.matcher(this).matches() && Pattern.compile(phoneLengthRE).matcher(this).matches()
fun String.isValidBirth(): Boolean = this.isNotEmpty() && Pattern.compile(birthRE).matcher(this).matches()
